...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException("error: can't call this method now");
        }
    ...
    private void checkFinishedDependencies() {
        for (int element : _depends.successors(_vertex)) {
            if (_depends.getLabel(element).isUnfinished()) {
    ...
    private boolean outOfDate() {
        if (_time == null) {
            ...
            for (int element : _depends.successors(_vertex)) {
                if (_time < _depends.getLabel(element).getTime()) {
                    return true;
                }
            }
        }
